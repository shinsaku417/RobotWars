...
@implementation ShingekiNoKyojin10 {
...
- (void)run {
    ...
    while (true) {
        if(self.currentTimestamp-_lastHitTimestamp>30.f){
            NSLog(@"stalemate!");
            stale+= 2;
        ...
        // If the bullet hits enemy, bulletHitEnemy method runs => read comment for that method first
            ...
            if (_hitting && emergency != 1) {
                if (_p1 && _atTop) {
                    ...
                    if ((_currentAngle - (90 - (int)(_angle + 0.5))) < 0) {
                        [self turnGunLeft:abs(_currentAngle - (90 - (int)(_angle + 0.5)))];
                    } else {
                ...
                if (_p1 && !_atTop) {
                    ...
                    if (((int)(_angle + 0.5) - _currentAngle) < 0) {
                        [self turnGunRight:abs((int)(_angle + 0.5) - _currentAngle)];
                    } else {
                ...
                if (_shootLeft) {
                    [self shoot];
                    [self turnGunLeft:10-stale];
                    _currentAngle += 10-stale;
                }
                // If false, rotate to the right
                else {
                    [self shoot];
                    [self turnGunRight:10-stale];
                    _currentAngle -= 10-stale;
                }
                if (_currentAngle >= 90 - (60*emergency))
                {
                    _shootLeft = false;
                    //_shootRight = true;
                }
                if (_currentAngle <= 0 -(40*emergency)) {
                    _shootLeft = true;
                    //_shootRight = false;
                }
